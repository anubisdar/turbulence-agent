...
Kept at the project root rather than under app/, because it is not part of
the application: importing it as a module would run these lines at startup.
...
# A validator on the request model rather than on the module. Vulture
# matches on token names, so the attribute only has to be spelled - but it
# has to be spelled on something that has it.
api.CorridorSearchBody.origin_and_destination_must_differ
api.CorridorSearchBody.clamped
