...
Four classes, and the last two are the point:
  ...
  TestKnownFalsePositives  - correct paragraphs this validator still
                             discards, the mechanism they share, and a
                             guard against the repair that looks obvious
                             and opens a hole.
...
class TestKnownFalsePositives:
    ...
    @pytest.mark.xfail(strict=True, reason=(
        "the enumeration tail 'or severe on this route' is 3+ words, so "
        "_clauses leaves it as its own clause and the governing negation "
        "stays behind in the previous one"))
    def test_denial_with_a_trailing_phrase_is_accepted(self):
        text = ("There is no basis in the available data to characterize "
                "conditions as light, moderate, or severe on this route. "
                "Nothing is known about the air you will be flying through. "
                "That is not the same as calm air.")
        v = verdict(text)
        assert v.ok, v.reasons

    @pytest.mark.xfail(strict=True, reason=(
        "without the Oxford comma the tail is 'moderate or severe', three "
        "words, so it is not merged back into the clause carrying 'no "
        "basis'"))
    def test_denial_without_an_oxford_comma_is_accepted(self):
        text = ("There is no basis in the available data to characterize "
                "conditions as light, moderate or severe. Nothing is known "
                "about the air you will be flying through. That is not the "
                "same as calm air.")
    ...
    @pytest.mark.xfail(strict=True, reason=(
        "same clause-splitting defect reached through a different negation "
        "cue - the problem is structural, not specific to 'no basis'"))
    def test_a_differently_worded_denial_is_accepted(self):
        text = ("No report supports calling the conditions light, moderate, "
                "or severe at your cruise altitude. Nothing is known about "
                "the air on this route. That is not the same as calm air.")
        v = verdict(text)
        assert v.ok, v.reasons
    ...
    def test_the_root_cause_is_the_clause_split(self):
        """White-box: pin the mechanism the four failures above share.

        Not a red-team case. It is here so that whoever fixes the rule can
        see the split that produces the false positive, and so the fix can
        be checked against the mechanism rather than against the symptom.
        """
        ...
        split = _clauses("there is no basis to call it light, moderate, "
                         "or severe on this route.")
        assert len(split) == 2, split
        assert "severe" in split[-1]
        assert "no basis" not in split[-1], (
            "the negation governing 'severe' is stranded in the previous "
            "clause, which is why _denied cannot see it")
